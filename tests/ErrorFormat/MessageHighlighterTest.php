...
final class MessageHighlighterTest extends TestCase
{
    ...
    public static function provideHighlightCases(): iterable
    {
        yield 'expects, given' => [
            ...
            'Parameter #1 $id of method Foo::bar() expects <fg=green;options=bold>int</>, <fg=yellow;options=bold>string</> given.',
        ];
        ...
        yield 'should return, but returns' => [
            ...
            'Method Foo::bar() should return <fg=green;options=bold>int</> but returns <fg=yellow;options=bold>string</>.',
        ];

        yield 'does not accept' => [
            'Property Foo::$bar (int) does not accept string.',
            'Property Foo::$bar (<fg=green;options=bold>int</>) does not accept <fg=yellow;options=bold>string</>.',
        ];
        ...
        yield 'nested generics with commas on both sides' => [
            ...
            'Parameter #1 $items of function process expects <fg=green;options=bold>array\<int, string\></>, <fg=yellow;options=bold>array\<string, int\></> given.',
        ];

        yield 'array shapes' => [
            'Parameter #1 $user of function save expects array{id: int, name: string}, array{id: string, name: int} given.',
            'Parameter #1 $user of function save expects <fg=green;options=bold>array{id: int, name: string}</>, <fg=yellow;options=bold>array{id: string, name: int}</> given.',
        ];

        yield 'callable types' => [
            'Method Foo::bar() should return callable(int, string): void but returns callable(string, int): int.',
            'Method Foo::bar() should return <fg=green;options=bold>callable(int, string): void</> but returns <fg=yellow;options=bold>callable(string, int): int</>.',
        ];

        yield 'does not accept with an array shape' => [
            'Property Foo::$bar (array{id: int, name: string}) does not accept array{id: string}.',
            'Property Foo::$bar (<fg=green;options=bold>array{id: int, name: string}</>) does not accept <fg=yellow;options=bold>array{id: string}</>.',
        ];
